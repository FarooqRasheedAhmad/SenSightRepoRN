...
export default class ProgressBar extends Component {
    ...
    render() {
        const { borderColor, borderRadius,
            borderTopLeftRadius, borderTopRightRadius,
            borderWidth, children, color, height, style, unfilledColor, width,
            ...restProps
        } = this.props;

        const innerWidth = Math.max(0, width || this.state.width) - borderWidth * 2;
        const containerStyle = {
            width, overflow: 'hidden', backgroundColor: unfilledColor,
            borderWidth, borderColor: borderColor || color,
