...
export const Assessments = props => {
  ...
  const doAdd = async (values) => {
    ...
    sendRequest({ uri: ADD_ASSESMENT, method: 'post', body: JSON.stringify({ ...values, score:Number(values.score)}), debug:false }).then(result => {
      setBusy(false);
      setShowModal(false);
  ...
  const getFilteredData = args => {
    ...
    if (searchKw) return data.filter(itm => {
      return itm.assessmentName ? itm.assessmentName.toLowerCase().includes(searchKw.toLowerCase()) : false;
    })
  }

  return (
    <View style={{ flex: 1, backgroundColor: theme.colors.colorPrimary, paddingTop: Platform.OS == 'ios' ? 44 : 0 }}>
      <NavigationHeader title={'Assessment Results'} leftText={'Back'} navigation={props.navigation} />
      ...
      <FlatList
        ...
        renderItem={({ item }) => (<AssessmentsItem {...item} onPress={() => setShowModal({ type: 'details', data: item })} />)}
      ...
      <Modal animationType="slide" transparent={true} visible={showModal !== false} onRequestClose={() => setShowModal(false)} >
        <TouchableOpacity onPress={() => setShowModal(false)} style={{ backgroundColor: "rgba(0,0,0,0.5)", flex: 1, alignItems: "center", justifyContent: "center", padding: 5 }} activeOpacity={1}>
          ...
          {showModal && showModal.type == 'details' && <>
            <View style={{ paddingHorizontal: 15, paddingVertical: 20, backgroundColor: "#FFF", borderRadius: 10, width: "100%" }}>
              <Row style={styles.detailRow}>
                <Col flex={150}><Text style={styles.detailRowText}>Assessments Name: </Text></Col>
                <Col flex="auto"><Text style={{ ...styles.detailRowText, fontWeight: "bold" }}>{showModal.data.assessmentName}</Text></Col>
              </Row>
              <Row style={styles.detailRow}>
                <Col flex={150}><Text style={styles.detailRowText}>Date: </Text></Col>
                <Col flex="auto"><Text style={styles.detailRowText}>{moment(showModal.data.assessDate || "").format('MMMM D, YYYY')}</Text></Col>
              </Row>
              <Row style={styles.detailRow}>
                <Col flex={150}><Text style={styles.detailRowText}>Score: </Text></Col>
                <Col flex="auto"><Text style={styles.detailRowText}>{showModal.data.score}</Text></Col>
              </Row>
              <Row style={styles.detailRow}>
                <Col flex={150}><Text style={styles.detailRowText}>Remarks: </Text></Col>
          ...
          {showModal && showModal.type == 'form' && <AssessmentsForm busy={busy} onSubmit={doAdd} />}
